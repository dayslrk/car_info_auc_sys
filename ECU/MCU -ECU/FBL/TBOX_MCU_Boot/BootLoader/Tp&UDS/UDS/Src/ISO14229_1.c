...
{
    {0xF183, 10, READONLY , EEPROM_DID_0xF183, ZERO_LEVEL      , EEPROM_DID_Writeable},
    {0xF187, 5 , READONLY , EEPROM_DID_0xF187, PROGRAMING_LEVEL, EEPROM_DID_Writeable},
    {0xF18C, 16, READONLY , EEPROM_DID_0xF18C, ZERO_LEVEL      , EEPROM_DID_Writeable},
	{0xF190, 17, READONLY , EEPROM_DID_0xF190, ZERO_LEVEL      , EEPROM_DID_Writeable},
    {0xF191, 4 , READONLY , EEPROM_DID_0xF191, ZERO_LEVEL      , EEPROM_DID_Writeable},
    {0xF1A0, 6 , READONLY , EEPROM_DID_0xF1A0, ZERO_LEVEL      , EEPROM_DID_Writeable},
	{0xF1A1, 6 , READONLY , EEPROM_DID_0xF1A1, ZERO_LEVEL      , EEPROM_DID_Writeable}
};

static const u8 ECUHardwareNumber[4] = {0x00, 0x00, 0x00, 0x01};
static const u8 ECUPartNumber[5] = {0x00, 0x00, 0x00, 0x01, 0x01};
//static const u8 ECUBootSoftRefNum[10] = {'2', '0', '0', '6', '2', '0', '0', '0', '0', '1'};
static const u8 ECUBootSoftRefNum[10] = {0x02, 0x00, 0x00, 0x06, 0x02, 0x00, 0x00, 0x00, 0x00, 0x01};
static const u8 ECUSerialNumber[16] = {0x00};
static const u8 ECUVIN[17] = {'L', 'F', 'V', '2', 'A', '2', '1', 'J', '9', 'L', '3', '0', '0', '0', '0', '0', '1'};
...
void UDS_Init(void)
{
    ...
    u8 FlashECUHardwareNumber[4] = {0};
    u16 Index = 0;
    bool BootSoftNumRight = FALSE;
    u8 FlashECUBootSoftRefNum[12] = {0};
    u8 FlashECUPartNumber[5] = {0};
    ...
    Eeprom_Read(EEPROM_COMCONTROL_NORMALMESSAGEADDR, &NormalMessageEnableStatues, (u16)1);/*从内存中读取0x28服务的控制情况*/
    /*普通应用报文接收使能或禁止*/
    ...
    Eeprom_Read(EEPROM_CURRENT_SESSION, &CurrentSession, (u16)1);/*保持APP的02会话*/
    if (DEFAULT_SESSION != CurrentSession && PROGRAM_SESSION != CurrentSession && EXTENDED_SESSION != CurrentSession)
    {
        CurrentSession = DEFAULT_SESSION;
    }
    else if (PROGRAM_SESSION == CurrentSession)
    {
        i = 0xFF;
        Eeprom_Write(EEPROM_CURRENT_SESSION, (u8 *)&i, (u16)1);/*清除编程会话标志位，防止一直停留在BootLoader*/
    }

    for (Index = 0; Index < DID_COUNT; ++Index)/*检查Bootloader写死DID*/
    {
        if (0xF183 == DIDList[Index].ID)
        {
            Eeprom_Read(DIDList[Index].EEProm_DataIndex, FlashECUBootSoftRefNum, (u16)(DIDList[Index].Size));
            for (i = 0; i < DIDList[Index].Size; ++i)
            {
                BootSoftNumRight = (FlashECUBootSoftRefNum[i] == ECUBootSoftRefNum[i])?TRUE:FALSE;
            ...
            if (FALSE == BootSoftNumRight)
            {
                Eeprom_Write(DIDList[Index].EEProm_DataIndex, (u8 *)ECUBootSoftRefNum, (u16)(DIDList[Index].Size));
            }
            else
            {
                ;
        ...
        else if (0xF191 == DIDList[Index].ID)
        {
            Eeprom_Read(DIDList[Index].EEProm_DataIndex, FlashECUHardwareNumber, (u16)(DIDList[Index].Size));
            for (i = 0; i < DIDList[Index].Size; ++i)
            {
                BootSoftNumRight = (FlashECUHardwareNumber[i] == ECUBootSoftRefNum[i])?TRUE:FALSE;
                if (FALSE == BootSoftNumRight)
                {
                    break;
                }
            ...
            if (FALSE == BootSoftNumRight)
            {
                Eeprom_Write(DIDList[Index].EEProm_DataIndex, (u8 *)ECUHardwareNumber, (u16)(DIDList[Index].Size));
            }
            else
            {
                ;
        ...
        else if (0xF187 == DIDList[Index].ID)
        {
            Eeprom_Read(DIDList[Index].EEProm_DataIndex, FlashECUPartNumber, (u16)(DIDList[Index].Size));
            for (i = 0; i < DIDList[Index].Size; ++i)
            {
                BootSoftNumRight = (FlashECUPartNumber[i] == ECUPartNumber[i])?TRUE:FALSE;
                if (FALSE == BootSoftNumRight)
                {
                    break;
                }
            ...
            if (FALSE == BootSoftNumRight)
            {
                Eeprom_Write(DIDList[Index].EEProm_DataIndex, (u8 *)ECUPartNumber, (u16)(DIDList[Index].Size));
            }
            else
            {
                ;
        ...
        else if (0xF18C == DIDList[Index].ID)
        {
            Eeprom_Read(DIDList[Index].EEProm_DataIndex, FlashECUPartNumber, (u16)(DIDList[Index].Size));
            for (i = 0; i < DIDList[Index].Size; ++i)
            {
                BootSoftNumRight = (FlashECUPartNumber[i] == ECUSerialNumber[i])?TRUE:FALSE;
                if (FALSE == BootSoftNumRight)
                {
                    break;
                }
            ...
            if (FALSE == BootSoftNumRight)
            {
                Eeprom_Write(DIDList[Index].EEProm_DataIndex, (u8 *)ECUSerialNumber, (u16)(DIDList[Index].Size));
            }
            else
            {
                ;
        ...
        else if (0xF190 == DIDList[Index].ID)
        {
            Eeprom_Read(DIDList[Index].EEProm_DataIndex, FlashECUPartNumber, (u16)(DIDList[Index].Size));
            for (i = 0; i < DIDList[Index].Size; ++i)
            {
                BootSoftNumRight = (FlashECUPartNumber[i] == ECUVIN[i])?TRUE:FALSE;
                if (FALSE == BootSoftNumRight)
                {
                    break;
                }
            ...
            if (FALSE == BootSoftNumRight)
            {
                Eeprom_Write(DIDList[Index].EEProm_DataIndex, (u8 *)ECUVIN, (u16)(DIDList[Index].Size));
            }
            else
            {
                ;
...
void UDS_RxIndication(u8 *Data, u16 Len, u8 FrameType, u8 Channel, u8 apptype)
{
    if (NULL != Data)
    {
        ...
        UDS_TimerRun(&SessionTimer, SESSIONTIMEOUT, UDS_SessionTimeout);/*每收到一条诊断指令，会刷新会话管理的timer,ISO15765中也有刷新，此处为MPU自诊断刷新timer，不影响can诊断帧*/
    }
    else
    {
        ;/*数据指针为空*/
            ...
            else if (Subfunction == UnlockList[Index].KeyID)/*验证钥匙*/
            {
                if (5 == Len)/*长度正确*/
                {
                    if (WAIT_REC_KEY == UnlockStep)/*正在等待钥匙的验证*/
                    {
                        ...
                        Eeprom_Read(EEPROM_UNLOCKFAILEDCOUNTADDR, &UnlockFailedCounter, (u16)1);/*读取EEProm中验证钥匙失败次数*/
                        ...
                        Eeprom_Write(EEPROM_UNLOCKFAILEDCOUNTADDR, &UnlockFailedCounter, (u16)1);/*将验证失败次数写入EEProm*/
                    }
...
void UDS_GeneratSeed(u8 *Seed)
{
    if (NULL != Seed)
    {
        ...
        Seed[2] = UDS_rand();
        Seed[3] = UDS_rand();
        Log(LOG_INFO, "Seed: 0x%02x %02x %02x %02x", Seed[0],Seed[1],Seed[2],Seed[3]);
    }
    else
    {
        ;
...
{	
	...
	if(NULL != pSeed && NULL != pKey)
	{
    	...
    	Log(LOG_INFO, "key: 0x%02x %02x %02x %02x", pKey[0],pKey[1],pKey[2],pKey[3]);
    }
    else
    {
        ;/*指针为空，错误*/
...
void UDS_CommunicationControl(u8 Subfunction, u8 ControlType)
{
    ...
    Eeprom_Write(EEPROM_COMCONTROL_NORMALMESSAGEADDR, &Subfunction, (u16)1);/*存储通信控制状态，断电仍保持*/
}
...
/*长度正确*/
    {
        ...
        if (TRUE == DIDExist)/*DID存在*/
        {
            if (READONLY == DIDList[Index].RWAttribute || READWRITE == DIDList[Index].RWAttribute)/*DID属性为可读*/
            {
                ...
                Eeprom_Read(DIDList[Index].EEProm_DataIndex, &(PosRespBuffer.Data[3]), (u16)(DIDList[Index].Size));/*从内存中读取DID的值到正响应buffer*/
        ...
        if (TRUE == DIDExist)/*DID存在*/
        {
            if (DIDList[Index].SecurityLevel == CurrentSecurityLevel || ZERO_LEVEL == DIDList[Index].SecurityLevel)/*满足写DID要求的安全等级*/
            {
                if (WRITEONLY == DIDList[Index].RWAttribute || READWRITE == DIDList[Index].RWAttribute)/*该DID可写*/
                {
                    ...
                    {
                        Eeprom_Read(DIDList[Index].EEProm_WritableIndex, &DIDWritable, (u16)1);/*从EEProm中读取flag，查询该DID是否可写（有的DID只能写一次）*/
                        if (0 != ((DIDWritable >> Index) & 0x01))
/*该DID仍可写*/
                        {
                            /*WriteResult = */Eeprom_Write(DIDList[Index].EEProm_DataIndex, &Data[2], (u16)(DIDList[Index].Size));
                            ...
                            if (DID == 0xFFFF)/*只能写一次的DID,BootLoader模式情况下没有这种DID*/
                            {
                                DIDWritable &= ~(0x01 << Index);
                                Eeprom_Write(DIDList[Index].EEProm_WritableIndex, &DIDWritable, (u16)1);/*将本DID的可写修改为不可写*/
                            }
                            else
                            {
                                ;
...
void UDS_EraseMemory(void)
{
    ...
    Eeprom_Write(EEPROM_BACKUPJUMPTOAPPADDRESS, AppAddress, 4);/*存入备份APP的跳转地址*/

    JumpAPPFailedCount = 0;
    Eeprom_Write(EEPROM_JUMPTOAPPFAILEDCOUNT, &JumpAPPFailedCount, (u16)1);/*设置APP程序启动失败次数为0，待刷新完毕后，每次重启失败+1*/
#endif
...
void UDS_APPRollBack(void)
{
    ...
    Eeprom_Read(EEPROM_BACKUPJUMPTOAPPADDRESS, AppAddress8, 4);
    Eeprom_Write(EEPROM_JUMPTOAPPADDRESS, AppAddress8, 4);
            ...
            (TRANSFER_MEMORYSIZEBYTE == (Data[1] >> 4) && TRANSFER_ADDRESSBYTE == (Data[1] & 0x0f)))
        {
            if (10 == Len)/*长度正确*/
            {
                ...
                if (ReprogramMemoryAddress >= MEMORY_SATRTADDRESS && (ReprogramMemoryAddress + ReprogramMemoryLength) <= MEMORY_ENDADDRESS)
                {
                    ...
                    Eeprom_Write(EEPROM_JUMPTOAPPADDRESS, AppAddress, 4);/*存入APP跳转地址*/
                    ...
                    PosRespBuffer.Data[2] = (MAX_UDS_RCV_Buff_Size >> 8) & 0xff;
                    PosRespBuffer.Data[3] = MAX_UDS_RCV_Buff_Size & 0xff;
        ...
        if (TRUE == Downloading)/*34服务已成功请求*/
        {
            if (Data[0] == (u8)(BlockIndex + 1))/*BlockIndex正确*/
            {
                ...
                if ((ProgramReceivedLength + Len - 2) <= ReprogramMemoryLength)/*已下载长度未超过34服务指定下载长度*/
                {
                    ...
                    CS = UDS_CheckSum(&Data[1], Len - 2);
                    if ((CS == Data[Len -1]) && !Flash_Write(ReprogramMemoryAddress + ProgramReceivedLength, (u32 *)&Data[1], (u32)(Len - 2) / 4))
                    {
                        /*填充正响应*/
                        PosRespBuffer.Data[0] = ServiceList[ServiceIndex].ServiceName + 0x40;
...
*******************************************************************************/
u8 UDS_CheckSum(u8 *Data, u16 Len)
{ 
    u16 i = 0;
    u8 ret = 0;
