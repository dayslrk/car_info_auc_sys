...
const DIDConfigure_Type DIDList[DID_COUNT] = 
{
    ...
    {0xF187, 5 , READONLY , 0x000C, PROGRAMING_LEVEL, 0x0011},
    {0xF18C, 16, READONLY , 0x0012, ZERO_LEVEL      , 0x0022},
	{0xF190, 17, READONLY , 0x0023, ZERO_LEVEL      , 0x0034},
...
void UDS_Init(void)
{
    ...
    AT24Cxx_Read(CURRENT_SESSION, &CurrentSession, (u16)1);/*保持APP的02会话*/
    ...
    {
        u8 i = 0xFF;
        AT24Cxx_Write(CURRENT_SESSION, &i, (u16)1);/*清除编程会话标志位，防止一直停留在BootLoader*/
    }
    ...
    for (Index = 0; Index < DID_COUNT; ++Index)/*检查Bootloader写死DID*/
    {
        ...
        {
            AT24Cxx_Read(DIDList[Index].EEPromAddr, FlashECUBootSoftRefNum, (u16)(DIDList[Index].Size));
            ...
            if (FALSE == BootSoftNumRight)
            {
                AT24Cxx_Write(DIDList[Index].EEPromAddr, (u8 *)ECUBootSoftRefNum, (u16)(DIDList[Index].Size));
            }
            else
            {
                ;
        ...
        {
            AT24Cxx_Read(DIDList[Index].EEPromAddr, FlashECUHardwareNumber, (u16)(DIDList[Index].Size));
            for (i = 0; i < DIDList[Index].Size; ++i)
            {
            ...
            if (FALSE == BootSoftNumRight)
            {
                AT24Cxx_Write(DIDList[Index].EEPromAddr, (u8 *)ECUHardwareNumber, (u16)(DIDList[Index].Size));
            }
            else
            {
                ;
        ...
        {
            AT24Cxx_Read(DIDList[Index].EEPromAddr, FlashECUPartNumber, (u16)(DIDList[Index].Size));
            for (i = 0; i < DIDList[Index].Size; ++i)
            {
            ...
            if (FALSE == BootSoftNumRight)
            {
                AT24Cxx_Write(DIDList[Index].EEPromAddr, (u8 *)ECUPartNumber, (u16)(DIDList[Index].Size));
            }
            else
            {
                ;
