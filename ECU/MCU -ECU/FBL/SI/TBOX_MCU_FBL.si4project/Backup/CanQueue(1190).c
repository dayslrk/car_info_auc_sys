...
u8 CAN_Queue_Pop(pCAN_Msg_Type Data)
{
    ...
    else
    {
        ...
        /*Data->ID = CAN_Queue_Rec.Buffer[CAN_Queue_Rec.front].ID;
        Data->Len = CAN_Queue_Rec.Buffer[CAN_Queue_Rec.front].Len;
        for (i = 0; i < Data->Len; i++)
        {
            Data->Data[i] = CAN_Queue_Rec.Buffer[CAN_Queue_Rec.front].Data[i];
        }*/
        *Data = CAN_Queue_Rec.Buffer[CAN_Queue_Rec.front];
        /* 取出一次数据，队头前后挪动1 */
