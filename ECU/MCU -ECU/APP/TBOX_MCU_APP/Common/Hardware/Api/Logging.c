...
void Log(LOGGING_LEVEL level, char const *format, ...)
{
	...
	{
		case LOG_ERROR:
			Logging("APP_[%05d] %s-%d ERR:   %s", msec, __FUNCTION__,__LINE__,s_buffer);
			break;
		case LOG_WARN:
			Logging("APP_[%05d] %s-%d WARN:  %s",msec, __FUNCTION__,__LINE__, s_buffer);
			break;
		case LOG_DEBUG:
			Logging("APP_[%05d] %s-%d DEBUG: %s", msec, __FUNCTION__,__LINE__, s_buffer);
			break;
		default:
			Debug_SendMsg(s_buffer);
			break;
