 ...
 */
u8 FlashDriver_Write(u32 WriteAddr,u8 *pBuffer,u32 NumToWrite)	
{ 
	u8 status = 0;
	u32 endaddr=0;
	...
	{
		//在此处先使用4字节刷写
		while(WriteAddr+4 < endaddr)
		{
			if(FLASH_ProgramWord(WriteAddr,*((u32 *)pBuffer)) != FLASH_COMPLETE)//写入数据
			{ 
				break;	//写入异常
			}
			WriteAddr += 4;
 ...
 *  @retval	无
 */
void FlashDriver_Read(u32 ReadAddr, u8 *pBuffer, u32 NumToRead)   	
{
	u32 i;
	for(i=0;i < NumToRead;i++)
	{
		pBuffer[i] = *(vu8 *)ReadAddr;;//读取1个字节.
		ReadAddr += 1;//偏移1个字节.	
	}
}
