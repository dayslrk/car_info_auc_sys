...
extern DiagTimer_Type SessionTimer;/*会话超时Timer*/
/*******************************************************************************
* 函数名称: ISO15765_Init
...
void ISO15765_ReceivePacket(u8 Channel, pCAN_Frame_Type pCANFrame)
{
    ...
    if (DIAGFUNCRXID == pCANFrame->ID || DIAGCANRXID == pCANFrame->ID)
    {
        UDS_TimerRun(&SessionTimer, SESSIONTIMEOUT, UDS_SessionTimeout);/*每收到一条诊断指令，会刷新会话管理的timer*/
    }
    else
    {
        ;
    ...
    do{
        ...
        switch (CANFrame.Byte[0] >> 4)
        {
            case SF_FLAG:
                ...
                /*数据接收完成后,检查是否需要ecu处理，如需处理则上报请求到uds层*/
            ...
            case FF_FLAG:
                ...
                //CAN_Channel.ChanRecv[en_chan_addr].len += CANFrame.Byte[1];                
            ...
            case CF_FLAG:
                ...
                else/*SN正确*/
                {
                    ...
                    for (i = 1; i <= 7; ++i)
                    {
                        ...
                        CAN_Channel.ChanRecv[en_chan_addr].Data[CAN_Channel.ChanRecv[en_chan_addr].CurIndex] = CANFrame.Byte[i];
...
void ISO15765_CheckFrame(CHANNEL_RECV *pRecv)
{
    ...
    if( (pRecv->CanId == DIAGCANRXID || pRecv->CanId == DIAG_FUNC_ADDR) &&
        (pRecv->Sta == 2) && 0 != pRecv->len)
    {
        FrameType = pRecv->CanId == DIAGCANRXID ? DIAG_PHY: DIAG_FUNC;
...
void ISO15765_SendPacket(void)
{
    ...
    do{
        ...
        if( 3 == CAN_Channel.ChanSend.Sta)
        {
            if(0 == (CAN_Channel.ChanSend.WFCTimeout % 500)	)
            {
                CAN_Channel.ChanSend.Sta = 1;	//重发
            }

            if((CAN_Channel.ChanSend.WFCTimeout / 500) > 3)
            {
                //发送失败
                CAN_Channel.ChanSend.Sta = 4;	
            }
            CAN_Channel.ChanSend.WFCTimeout ++;
        }
        ...
        if(1 == CAN_Channel.ChanSend.Sta)
        {
            ...
            switch(CAN_Channel.ChanSend.CanId)
            {
                ...
                case DIAGCANTXID:
                    ...
                    memset(&(CAN_Channel.ChanRecv[EN_CHAN_PHY_REQ]), 0x00, sizeof(CAN_Channel.ChanRecv[EN_CHAN_PHY_REQ]));
...
//                delay_ms(2);		//防止网关路由丢包
        ...
        if( DIAGCANTXID == CAN_Channel.ChanSend.CanId)
        {
...
bool ISO15765_MakePacket(u32 CanId, u8 *Data, u16 Len)
{
    if (NULL != Data && 0 == CAN_Channel.ChanSend.Sta && Len <= MAX_UDS_RCV_Buff_Size )
    {
        ...
        CAN_Channel.ChanSend.WFCTimeout = 1;	//清空等待流控时间
