...
const DIDConfigure_Type DIDList[DID_COUNT] = 
{
    ...
    {0xF187, 5 , READONLY , EEPROM_DID_0xF187, ZERO_LEVEL      , EEPROM_DID_Writeable},
    {0xF18C, 16, READWRITE, EEPROM_DID_0xF18C, ZERO_LEVEL      , EEPROM_DID_Writeable},
    {0xF190, 17, READWRITE, EEPROM_DID_0xF190, EXTENDED_LEVEL  , EEPROM_DID_Writeable},
...
u8 AppSoftwareNumber[6] = {0, 0, 6, 0x20, 0x11, 0x04};/*存储接收的软件版本号*/
...
void UDS_Init(void)
{
    ...
    {
        if (0xF1A0 == DIDList[Index].ID)
        {
            Eeprom_Read(DIDList[Index].EEProm_DataIndex, FlashAppSoftwareNumber, (u16)(DIDList[Index].Size));
            ...
            if (FALSE == BootSoftNumRight)
            {
                Eeprom_Write(DIDList[Index].EEProm_DataIndex, AppSoftwareNumber, (u16)(DIDList[Index].Size));
            }
            else
            {
                ;
...
void UDS_EnterReprogramSession(void)
{
    ...
    Eeprom_Write(EEPROM_CURRENT_SESSION, &WriteSession, (u16)1);
    SystemReset();
}
...
void UDS_CheckApplicationDataValid(void)
{
    ...
    while (ProcessedSize < ReprogramMemoryLength)/*CRC还未计算完毕*/
    {
         ...
         Flash_Read(ReprogramMemoryAddress + ProcessedSize, Data, (u32)Size );/*读取成功*/
//         UDS_ByteConvert(Data, Size);
         LocalCRC = CRC32_Cal_Buffer(LocalCRC,Data, (u32)Size);/*调用硬件CRC计算接口*/
...
void UDS_APPRollBack(void)
{
    ...
    Result = Flash_Write(MEMORY_BACKUPSTARTADDRESS, (u8 *)MEMORY_BACKUPSTARTADDRESS, (u32)((256 * 1024) ));
...
{
    Flash_Write(MEMORY_BACKUPSTARTADDRESS, (u8 *)MEMORY_SATRTADDRESS, (u32)(256 * 1024));
...
void UDS_Service0x36Handle(u16 Len, u8 FrameType, u8 *Data, u8 ServiceIndex)
{
    ...
    if (Len >= 2 && (Len - 2) <= DOWNLOAD_MAXBUFSIZE )/*长度大于最小要求长度，且小于34服务发送的最大长度*/
    {
        if (TRUE == Downloading)/*34服务已成功请求*/
        {
            if (Data[0] == (u8)(BlockIndex + 1))/*BlockIndex正确*/
            {
                ...
                if ((ProgramReceivedLength + Len - 2) <= ReprogramMemoryLength)/*已下载长度未超过34服务指定下载长度*/
                {
                    ...
                    Num = (u32)(Len - 2);
                    if ((CS == Data[Len -1]) && !Flash_Write(ReprogramMemoryAddress + ProgramReceivedLength, (u8 *)&Data[1], Num))
                    {
                        /*填充正响应*/
                        PosRespBuffer.Data[0] = ServiceList[ServiceIndex].ServiceName + 0x40;
...
/*******************************************************************************
* 函数名称: UDS_SaveMpuVersion
* 功能描述: 存入MPU版本号
* 输入参数: Data：版本号 Len：版本号长度
* 输出参数: 无
* 返回参数: 无
* 备    注： 
*******************************************************************************/
void UDS_SaveMpuVersion(u8 *Data, u16 Len)
{
    u8 Index = 0;
    u8 FlashAppSoftwareNumber[6] = {0};
    ...
    for (Index = 0; Index < DID_COUNT; ++Index)/*检查App软件版本号*/
        {
        if (0xF1A1 == DIDList[Index].ID)
        {
            Eeprom_Read(DIDList[Index].EEProm_DataIndex, FlashAppSoftwareNumber, (u16)(DIDList[Index].Size));
            if(0 == memcmp(FlashAppSoftwareNumber, Data, 6 ))
            {
                break;
            }
            
            Eeprom_Write(DIDList[Index].EEProm_DataIndex, Data, (u16)(DIDList[Index].Size));
            break;
        }
    }
}
