    ...
    do{
        ...
        switch (CANFrame.Byte[0] >> 4)
        {
            case SF_FLAG:
                ...
                memcpy(CAN_Channel.ChanRecv[en_chan_addr].Data, &(CANFrame.Byte[1]), CAN_Channel.ChanRecv[en_chan_addr].len);

#if 1   //测试代码             
                if(0x74 == CANFrame.Byte[1])
                {
                    Log(LOG_INFO,"\n");
                    break;
                }
                ...
                if (CAN_SendData(CAN_Channel.Channel, &CANFrame))
                {
                   //发送失败
                    ...
                    if(CAN_Channel.ChanSend.RetryTimes > MAX_RETRYTIMES)
                    {
                        CAN_Channel.ChanSend.Sta = 4;        //发送失败，发送完成
                        CAN_Channel.ChanSend.RetryTimes = 0;
                        UDS_TxConfirmation(CAN_Channel.ChanSend.Data[0],CAN_Channel.ChanSend.Data[1], Tx_FAILURE);
                    }
                }
                else
                {
                    //单帧发送成功，
                    CAN_Channel.ChanSend.RetryTimes = 0;
                    UDS_TxConfirmation(CAN_Channel.ChanSend.Data[0],CAN_Channel.ChanSend.Data[1], Tx_SUCCESS);
