		...
		for(j = 0; j< send_quque[i].len; j++)
		{
			...
			if(EN_TSTA_SUCC == send_quque[i].Node[index].ENState)
			{
				...
				if(EN_MCU_APP1 == send_quque[i].Node[index].msg.Sender)
				{
					...
					{
						UDS_TxConfirmation(send_quque[i].Node[index].msg.Data[4], send_quque[i].Node[index].msg.Data[5],Tx_SUCCESS );
					}
					else
					{
						UDS_TxConfirmation(send_quque[i].Node[index].msg.Data[3], send_quque[i].Node[index].msg.Data[4],Tx_SUCCESS );
					}
				}
			...
			if(EN_TSTA_FAIL == send_quque[i].Node[index].ENState)
			{
				if(EN_MCU_APP1 == send_quque[i].Node[index].msg.Sender)
				{
					if(send_quque[i].Node[index].msg.Data[2] >= 0x10)		//多帧
					{
						UDS_TxConfirmation(send_quque[i].Node[index].msg.Data[4], send_quque[i].Node[index].msg.Data[5],Tx_FAILURE );
					}
					else
					{
						UDS_TxConfirmation(send_quque[i].Node[index].msg.Data[3], send_quque[i].Node[index].msg.Data[4],Tx_FAILURE );
					}
				}
...
u32 msgq_dist()
{
	...
	for(type = EN_MCU_APP1; type < EN_MCU_APP1 + (RECV_NUM * 2); type += 2)
	{
		...
		{
                  Log(LOG_INFO, "Recv data,type: %d.\n", type);
		    switch(type)
		    {
		        //接收到数据.
    			case EN_MCU_APP1:
                              //收到诊断指令。
                              //TODO:再次区分当前的诊断指令的CANID。
                              Log(LOG_INFO, "EN_MCU_APP1 Recv data.\n");
                              ...
                              else if(DIAGFUNCRXID == CanId)
                              {
                                      ...
                                      //在此次将消息插入到TP的采集任务中
                                      AddUdsDiagTaskFromMpu(CanId, recv_buff.mdata.mlen -index ,recv_buff.mdata.mbuff + index, recv_buff.mdata.apptype);
                              }
                              else if(0x700 == (CanId & 0x700) && 0x708 != (CanId & 0x708))
                              {
                                      //只诊断其他ECU
                ...
                case EN_MCU_APP2:
                    ...
                    break;
                case EN_MCU_APP3:
                    if ((CALLBACK)APPCALLBACK3 != (CALLBACK)NULL)
                    {
                        ((CALLBACK)APPCALLBACK3)(&recv_buff);
                    }
                    else
                    {
                        ;
